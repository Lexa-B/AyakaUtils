...
class ChatAyaka(BaseChatModel):
    ...
    def _create_message_dicts(self, messages: List[Any]) -> List[Dict[str, Any]]:
        ...
        for message in messages:
            message_dict = {"role": message.type, "content": message.content}
...
class AyakaMessagePromptTemplate(BaseMessagePromptTemplate):
    ...
    add_response_marker: bool = Field(
        default=True, description="Whether to add the response marker"
    )
    template_variables: List[str] = Field(
        default_factory=list, description="Input variables for the template"
    )
