...
from ayaka_utils.Classes.ChatAyaka import ChatAyaka
...
def apply_model_configs(
    ...
    for model in models:
        ...
        if isinstance(model, (ChatNVIDIA, ChatAyaka, ChatOpenAI)):
            ...
            # Determine if it's chat, instruct, reasoning, or deep_context, or a custom model
            ...
            if model_name == "chat" or model_name == "instruct" or model_name == "reasoning" or model_name == "deep_context":
...
def get_configured_model(
    model_type: str, config_file: str
    ...
    if model_type in ["chat", "instruct", "reasoning", "deep_context"]:
        ...
        try:
            model_function = config["Model_Functions"][f"{model_type}"]
            model_name = config["LLM_Models"][f"{model_type}_model"]

            if model_function == "ChatAyaka":
                model = ChatAyaka(
                    nvidia_api_url=config["NetLocations"][f"{model_type}_base_url"],
                    model=model_name,
                )
            elif model_function == "ChatOpenAI":
                # For OpenAI, we use api_key from environment variable OPENAI_API_KEY
                ...
                model = ChatNVIDIA(
                    base_url=config["NetLocations"][f"{model_type}_base_url"],
                    model=model_name,
                )
            # Apply configuration using the main function
            return apply_model_configs([model], config_file)[0]
...
def get_configured_provider(
    provider_name: str, config_file: str = "./Configs/Default.ModelConfig.jsonc"
) -> Union[ChatNVIDIA, NVIDIAEmbeddings, ChatAyaka, ChatOpenAI]:
    """
    Create and configure a new model instance.
    ...
    if provider_name in providers:
        ...
        raise ValueError(f"Unknown model type: {provider_name}")
